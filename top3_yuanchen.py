...
# pairs = "IEEE/pairs"
# log = "IEEE/exp3_tnum_noweight/log.txt"
# attribute_matrix = "IEEE/exp3_tnum_noweight/attribute_M.txt"
label_index = "IEEE/label-index.txt"
paper_title = "IEEE/id_title.csv"

topic_fn = "yuhao2/paperId and topics.csv"
cite = "yuhao2/referee_referers.csv"
pairs = "yuhao2/pairs"
log = "yuhao2/exp3_top3/log.txt"
attribute_matrix = "yuhao2/exp3_top3/attribute_M.txt"
att_matrix = {}
test_top3 = {}
...
def read_attributes():
    ...
    for row in csv_f:
        ...
        for x in l:
            if x>0:
                att_list[i] = x
                att_matrix[int(row[0]),i] = x
                # if int(row[0]) in tnum:
                #     tnum[int(row[0])] += 1
                # else:
                #     tnum[int(row[0])] = 1
                i+=1
        att_ordered = od(sorted(att_list.items(), key = lambda t: t[1], reverse=True))
        # for a in att_ordered: print a, att_ordered[a]
        i = 0
        for k in att_ordered:
            if i>2:
                break
            test_top3[id, k] = att_ordered[k]
            i+=1
        tnum[id] = i
    f.close()
...
def initAttributeMatrix():
    ...
    with open(topic_fn, "r") as f1, open(label_index, "r") as Flabel, open(paper_title,"r") as Ftitle: #, open(attribute_matrix,"w") as f2:
        lines = Flabel.readlines()
        for l in lines:
            labels.append(l)
        # print labels

        lines = Ftitle.readlines()
        for l in lines:
            string = l.strip().split("	")
            # print string[0]
            # print string[1]
            titles[int(string[0])] = string[1]
...
def read_trainset_attributes(train_set):
    ...
    for item in train_set:
        ...
        for t in same_topics:
            if att_matrix[(id1,t)] > filtervalue and att_matrix[(id2,t)]> filtervalue:
                if t in x_dict:
...
def bivariable_normal_model(x_dict,y_dict,logFile):
    ...
    for t in x_dict.keys():
        ...
        try :
            if(len(x)>2):
                ...
                # for id in paperset:
                #     if (id,t) in att_matrix and att_matrix[id,t]>filtervalue:
                #         docs.append(id)
                ...
                for p1 in paperset:
                    for p2 in paperset:
                        if p1 != p2:
                            if (p1,t) in test_top3:
                                v1 = test_top3[p1,t]
                                if (p2, t) in att_matrix:
                                    v2 = att_matrix[p2, t]
                                else:
                                    v2 = 0
                                Mscore[p1, p2] = var[t].logpdf([v1, v2])
                            # else:
                            #     Mscore[p1, p2] = 0 # var[t].logpdf([v1, v2])
                ...
                for x in paperset:
                    for y in paperset:
                        if x!=y:
                            ...
                            # if (x,t) in att_matrix:
                            #     Mscore[x,y] *= att_matrix[x,t]

                            if (x,y) in Msum:
                                Msum[x,y]+=Mscore[x,y]
                                Mscoreoftopic[x, y]+=", "+str(t)+": "+str(Mscore[x,y])
                            else:
                                Msum[x,y] = Mscore[x,y]
                                Mscoreoftopic[x, y] = str(t) + ": " + str(Mscore[x,y])
    ...
    for key in Msum.keys():
        p = key[0]
        if p in tnum:
            print key, p
            Msum[key] = Msum[key]/tnum[p]
            Mscoreoftopic[key] += "///" + str(p)+" #: "+str(tnum[p])+"; "+str(Msum[key])
...
def trainmodels(logFile):
    ...
    """
    # fn0 = "IEEE/exp3_tnum_noweight/output.txt"
    fn0 = "yuhao2/exp3_top3/output.txt"
    with open(pairs, "r") as c ,  open(fn0, 'w') as output:
        ...
        for l in lines:
            ...
            p1 = int(pair.split(',')[0])
            p2 = int(pair.split(',')[1])
            if tnum[p1]>0 and tnum[p2]>0:
                paperset.add(p1)
                paperset.add(p2)
                test_sets[randint(0, 9)].append(pair)
        logFile.write("# paper: %i\n" % len(paperset))
        ...
        for n in range(0, 10):  # test_sets.keys():
            ...
            # scoreoftopic = "IEEE/exp3_tnum_noweight/%i_scorematrix.txt" % n
            scoreoftopic = "yuhao2/exp3_top3/%i_scorematrix.txt" % n
            # with open(scoreoftopic, 'w') as scoreFile:
            # np.savetxt(scoreFile, Msum, fmt='%.18g')
            # json.dump(remap_keys(Msum), open(scoreoftopic, 'w'))
            with open(scoreoftopic, 'w') as Fscore:
                for key in Msum.keys():
                    Fscore.write("%s:\t%f\t%s\n" %(str(key),Msum[key],Mscoreoftopic[key]))
...
def Test(paper, test_sets,logFile,output):
    ...
    for threshold in np.linspace(0,1,101): # range(0.6,0.91,0.01):
...
def calcPrecision(testset, testpair, threshold, output, logFile):
                ...
                if (p1,p2) in Msum:
                    if Msum[p1,p2] > threshold:
                        ...
                        if (str(p1)+','+str(p2) in testpair):
                            num += 1
                            # predicted.append(str(p1)+','+str(p2))
    ...
    #     if p not in predicted:
            # OutputAttributes(logFile, p)
            # logFile.write("not predicted: %s\n" % p)

    print("num of pairs\\total size:  %i   %i\n" % (num, total))
    if (total != 0):
        pcs = float(num) / total
        print("**********precision:", pcs)
        output.write("\t%i,\t%i,\t%f" % (num, total, pcs))
...
def calcRecall(testpair, threshold, output, logFile):
    ...
    for pair in testpair:
        ...
        if (Msum[p1, p2] > threshold):
            num += 1
            recalled.append(pair)

    logFile.write("lost: pairs\n" )
    for pair in testpair:
        if pair not in recalled:
            OutputAttributes(logFile,pair)
            logFile.write("not recalled: %s \n" % pair)
...
def OutputAttributes(logFile, pair):
    string = pair.split(',')

    p1 = int(string[0])
    p2 = int(string[1])
    logFile.write("%s\t%s\n" % (string, Mscoreoftopic[p1,p2]))
    # logFile.write("%s:\n%s\n%s\n%s\n" % (string, titles[p1], titles[p2], Mscoreoftopic[p1,p2]))
    # logFile.write("%i\n" % p1)
    for t in range(0, att_num):
        if (p1,t) in att_matrix:
            logFile.write("%i: %f; " % (t, att_matrix[p1,t]))
            # logFile.write("%s:\t%f\n " % (labels[t], att_matrix[p1,t]))
    logFile.write("---------\n")
    for t in range(0, att_num):
        if (p2,t) in att_matrix:
            logFile.write("%i: %f; " % (t, att_matrix[p2,t]))
            # logFile.write("%s:\t%f\n; " % (labels[t], att_matrix[p2,t]))
